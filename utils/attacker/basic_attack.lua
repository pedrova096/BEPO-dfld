...
function M:new(options)
  ...
  instance.state = {
    direction = nil,
    combo_number = options.combo_number,
  }
...
function M:on_attack_start()
  ...
  msg.post(self.attack_url, Msg.Attacker.START_ATTACK, {
    combo_number = combo_number,
    time = self.times.attack,
  })
end
...
function M:on_attack_exit()
  ...
  if (self.times.recovery or 0) <= 0 then
    self.state.direction = nil
    msg.post(self.attack_url, Msg.Attacker.START_COOLDOWN, {
      time = self.cooldown,
    })
    return
  end

  msg.post(self.attack_url, Msg.Attacker.START_RECOVERY, {
    time = self.times.recovery,
  })
end

function M:on_recovery_exit()
  if not self.attack_url then return end
